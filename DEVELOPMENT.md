# Développer et exécuter le backend

Le [README](README.md) présente le rôle de cette cible de démonstration, son core et le workflow d'intégration du code généré. Ce guide rassemble les commandes et les contrats techniques de l'application actuelle.

Le backend s'utilise avec [crud-fe](../crud-fe/DEVELOPMENT.md). Les changements répétés dans le domaine générable se préparent dans [Engine](../engine/DEVELOPMENT.md), puis sont transférés explicitement après comparaison.
...
Engine décrit le domaine RH ainsi que la structure commune de `Account` et `Role`. Le mot de passe peut figurer dans le contrat de création issu du formulaire ; son encodage et sa réinitialisation, la normalisation des identifiants de connexion et l'authentification restent des adaptations locales de `domain/admin` et de `core/security`. Les réponses ne contiennent jamais le hash du mot de passe.
...
La configuration autorise `POST /api/login` sans authentification, réserve `/api/admin/**` au rôle administrateur et exige une authentification pour les autres routes `/api/**`, dont les routes RH.

Le frontend sélectionne ses modules selon le rôle : administration des comptes pour `ROLE_ADMIN`, parcours RH pour `ROLE_GESTIONNAIRE_RH`. Cette sélection de présentation est distincte des règles appliquées par [SecurityConfiguration](src/main/java/app/core/security/config/SecurityConfiguration.java).
